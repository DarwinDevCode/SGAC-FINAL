...
public class DatabaseRoleAspect {
    ...
    public Object applyDatabaseRole(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        log.debug("[DB-ROLE] -> {} txActive={} thread={}", joinPoint.getSignature(), txActiveBefore,
        ...
        if (appRole != null && !appRole.isEmpty()) {
            ...
            // Map known non-existent roles to existing ones
            if ("role_decano".equals(dbRole)) {
                dbRole = "role_docente";
            }

            final String finalDbRole = dbRole;

            // SET LOCAL ROLE solo tiene efecto dentro de una transacción.
            // Si acá no hay tx activa, no intentamos cambiar el rol para evitar
            // inconsistencias.
            if (txActiveBefore) {
                try {
                    ...
                    session.doWork(connection -> {
                        try (Statement statement = connection.createStatement()) {
                            statement.execute("SET LOCAL ROLE " + finalDbRole);
                            log.debug("[DB-ROLE] Rol de BD cambiado exitosamente a: {}", finalDbRole);
                        }
                    });
                    roleChanged = true;
                } catch (Exception e) {
        ...
        try {
            ...
            if (roleChanged) {
                ...
                try {
                    ...
                    // evitar tocar conexión porque puede provocar 'No active transaction' en

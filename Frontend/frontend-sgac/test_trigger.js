...
async function run() {
  try {
    ...
    // Consultar triggers de la tabla
    const triggerQuery = await client.query(`
      SELECT trigger_name, event_manipulation, event_object_table, action_statement
      FROM information_schema.triggers
      WHERE event_object_table = 'evidencia_registro_actividad';
    `);
    
    console.log("== Triggers en evidencia_registro_actividad ==");
    console.log(JSON.stringify(triggerQuery.rows, null, 2));

    // Ver si hay funciones llamadas por esos triggers

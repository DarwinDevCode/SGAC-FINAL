  ...
  desactivarFacultad(facultad: Facultad): void {
    ...
    this.subs.add(
      this.usuarioService.desactivarFacultadCatalogo(facultad.idFacultad).pipe(
        ...
        next: () => {
          facultad.activo = !this.esActivo(facultad);
          this.facultades = this.facultades.map((f) => f.idFacultad === facultad.idFacultad ? { ...f, activo: facultad.activo } : f);
        },
  ...
  desactivarCarrera(carrera: Carrera): void {
    ...
    this.subs.add(
      this.usuarioService.desactivarCarreraCatalogo(carrera.idCarrera).pipe(
        ...
        next: () => {
          carrera.activo = !this.esActivo(carrera);
          this.carreras = this.carreras.map((c) => c.idCarrera === carrera.idCarrera ? { ...c, activo: carrera.activo } : c);
        },
  ...
  desactivarAsignatura(asignatura: AsignaturaCatalogo): void {
    ...
    this.subs.add(
      this.usuarioService.desactivarAsignaturaCatalogo(asignatura.idAsignatura).pipe(
        ...
        next: () => {
          asignatura.activo = !this.esActivo(asignatura);
          this.asignaturas = this.asignaturas.map((a) => a.idAsignatura === asignatura.idAsignatura ? { ...a, activo: asignatura.activo } : a);
        },
  ...
  desactivarPeriodo(periodo: PeriodoCatalogo): void {
    ...
    this.subs.add(
      this.usuarioService.desactivarPeriodoCatalogo(periodo.idPeriodoAcademico).pipe(
        ...
        next: () => {
          periodo.activo = !this.esActivo(periodo);
          this.periodos = this.periodos.map((p) => p.idPeriodoAcademico === periodo.idPeriodoAcademico ? { ...p, activo: periodo.activo } : p);
        },

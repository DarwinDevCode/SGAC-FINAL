  ...
  guardarUsuario() {
    ...
    this.subs.add(
      this.usuarioService.crear(this.form).subscribe({
        ...
        error: (error: any) => {
          const msg = (error?.error?.mensaje || '').toString();
          const normalizedMsg = msg.toLowerCase();

          if (normalizedMsg.includes('duplicate')) {
            alert('Error: Ya existe un usuario con esa cédula, correo o matrícula.');
            return;
          }

          if (normalizedMsg.includes('permission denied to create role')) {
            alert('No se pudo registrar el usuario porque la base de datos no tiene permisos para crear roles. ' +
              'Solicita ejecutar GRANT CREATEROLE al usuario de conexión o ajustar el procedimiento almacenado como SECURITY DEFINER.');
            return;
          }
